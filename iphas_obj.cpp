...
void iphas_obj::get_last_z(void)
{
	last_z_extn=(log(last_A)-last_bin->last_mu)/last_bin->last_sigma;
}

void iphas_obj::set_test_A_from_z(void)
{
	test_A=exp(last_z_extn*last_bin->test_sigma + last_bin->test_mu);
	last_bin->try_update(this);
}
