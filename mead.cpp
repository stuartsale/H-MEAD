...
// Start from backup_A_mean
	...
	previous_hyperprior_prob+=+log(pow(previous_internal_rel[0][0],2)*previous_internal_rel[0][1]/(pow(previous_rel[0][0],3)*previous_rel[0][3]*exp(pow(previous_rel[0][3],2))))-2*log(previous_rel[0][3]);
...
//	previous_hyperprior_prob+=-1*log(previous_internal_rel[0][0]);//-previous_internal_rel[0][0];//
	for (int i=1; i<150; i++)
	{
		...
		previous_hyperprior_prob+=+log(pow(previous_internal_rel[i][0],2)*previous_internal_rel[i][1]/(pow(previous_rel[i][0],3)*previous_rel[i][3]*exp(pow(previous_rel[i][3],2))))-2*log(previous_rel[i][3]);
	}
...
// First vary parameters for each star
		...
		{
			stars[it].star_try1(isochrones, l, b, previous_rel);
...
// Now vary hyper-parameters
		...
		current_hyperprior_prob+=+log(pow(internal_rel[0][0],2)*internal_rel[0][1]/(pow(new_rel[0][0],3)*new_rel[0][3]*exp(pow(new_rel[0][3],2))))-2*log(new_rel[0][3]);
		...
		{			
			...
			current_hyperprior_prob+=+log(pow(internal_rel[it][0],2)*internal_rel[it][1]/(pow(new_rel[it][0],3)*new_rel[it][3]*exp(pow(new_rel[it][3],2))))-2*log(new_rel[it][3]);
		}
...
// Accept or reject
		...
		if (it_num/1000.==floor(it_num/1000.)){cout << it_num << " " << global_current_prob << " " << internal_rel[50][0] << " " << new_rel[rel_length-1][0] << " " << current_hyperprior_prob << " " << accepted/it_num << " " << accepted << endl;}
